...
class ALBERTSentimentFineTuner(ALBERTSentimentBaseProcessor):
    ...
    class FisherInformationCalculator:
        def __init__(self, model, epsilon='auto'):
            self.model = model
            self.epsilon = epsilon
            self.fisher_diag = {}  # Dictionary to store diagonal FIM values

        def accumulate_gradients(self, loss):
            """
            Calculate gradients and accumulate the diagonal entries of the Fisher Information Matrix.
            """
            self.model.zero_grad()
            loss.backward(retain_graph=True)
            # Accumulate squared gradients to approximate FIM diagonally
        ...
        def compute_diagonal_covariance(self):
            """
            Compute the diagonal covariance matrix by inverting the accumulated Fisher Information diagonal.
            """
            if self.epsilon == 'auto':
                # Compute epsilon based on average Fisher information
                all_fisher_values = torch.cat([fisher_info.flatten() for fisher_info in self.fisher_diag.values()])
                average_fisher_info = torch.mean(all_fisher_values)
                epsilon_value = average_fisher_info * 1e-6
            else:
                epsilon_value = self.epsilon

            covariance_diag = {}
            for name, fisher_info in self.fisher_diag.items():
                covariance_diag[name] = 1.0 / (fisher_info + epsilon_value)
            return covariance_diag

        def save_covariance_matrix(self, filepath="cov_matrix.pt"):
            """
            Save the computed covariance matrix to a file using PyTorch's serialization.
            """
            covariance_diag = self.compute_diagonal_covariance()
            torch.save(covariance_diag, filepath)
    ...
    class CustomTrainer(Trainer):
        def __init__(self, *args, loss_function=None, fisher_calculator=None, **kwargs):
        ...
        def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
            ...
            if self.fisher_calculator is not None and model.training:
                self.fisher_calculator.accumulate_gradients(loss)
    ...
    def get_loss_and_hyperparams(self):
        ...
        if loss_function_name == 'compute_huber_loss':
            ...
            hyperparams = {
                'learning_rate': 1e-4,  # Adjusted from 10e-5 to 1e-4 for correct notation
                'batch_size': 10,
                'epochs': 3,
                'weight_decay': 0.05
            }
    ...
    def fine_tune(self, dataset=None, start_from_sentiment_context=None, save_fine_tune='yes',
        ...
        if self.model_size == 'xlarge':
            hyperparams['batch_size'] = max(1, hyperparams['batch_size'] // 2)  # Reduce batch size to fit in memory
        ...
        # Initialize the FisherInformationCalculator if cov_matrix is enabled
        if cov_matrix:
            fisher_calculator = self.FisherInformationCalculator(self.model, epsilon=epsilon)
            print("Fisher Information Matrix calculation is enabled.")
        else:
        ...
        if cov_matrix and fisher_calculator is not None:
            ...
            fisher_calculator.save_covariance_matrix(filepath="cov_matrix.pt")
    ...
    def estimate_ram_usage(self, batch_size, epochs):
        """
        Estimate RAM usage based on model size, batch size, and number of epochs.
        """
        # Estimate model size in bytes
        model_parameters = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
        model_size_bytes = model_parameters * 4  # assuming 32-bit floats
        model_size_gb = model_size_bytes / (1024 ** 3)

        # Estimate activation size per batch
        hidden_size = self.model.config.hidden_size
        seq_length = self.max_seq_length
        activation_size_bytes = batch_size * seq_length * hidden_size * 4
        activation_size_gb = activation_size_bytes / (1024 ** 3)

        # Total estimated RAM usage
        total_ram_usage_gb = model_size_gb + activation_size_gb

        # Multiply by a factor to account for optimizer states and other overhead
        total_ram_usage_gb *= 1.5
